...
if not df.empty:
    ...
    def format_km(event=None):
        text = km_var.get().replace('.', '').replace(',', '')
        if text.isdigit():
            formatted = f"{int(text):,}".replace(',', '.')
            km_var.set(formatted)
            km_entry.icursor(tk.END)

    km_var = tk.StringVar()
    km_var.trace("w", lambda *args: format_km())
    ...
    tk.Button(root, text="Fiyat Hesapla", command=lambda: tahmin_yap(), bg="#e67e22", fg="white", font=("Arial", 12, "bold")).grid(row=5, column=0, columnspan=2, pady=10, sticky="ew")
    ...
    def tahmin_yap():
        ...
        # --- GÜNCELLENEN YAŞ ALGORİTMASI (%3 BİLEŞİK DÜŞÜŞ) ---
        # Formül: Fiyat * (0.97 ^ Yaş)
        # Her yıl bir önceki yılın %97'si kalır (yani %3 düşer)
        yas_carpan = 0.97 ** yas
        
        fiyat_yas_sonrasi = sifir_fiyat * yas_carpan
        yas_kaybi = sifir_fiyat - fiyat_yas_sonrasi

        # --- KM ALGORİTMASI (4 Aşamalı) ---
        if km <= 100000:
            units = km / 10000
