...
if not df.empty:
    ...
    # --- GUI ---
    root = tk.Tk()
    root.title("Araba Fiyat Tahmini (Yeni KM Algoritması)")
    ...
    tk.Button(root, text="Tahmin Et", command=lambda: tahmin_yap(), bg="darkred", fg="white", font=("Arial", 12, "bold")).grid(row=5, column=0, columnspan=2, pady=10, sticky="ew")
    ...
    metrics_label = tk.Label(root, text=f"Model R²: {r2:.4f} | Algoritma: Her 10k KM'de %5 Kayıp + Yıllık %3 Yaş Kaybı", font=("Arial", 9))
    ...
    def tahmin_yap():
        ...
        # --- ADIM 2: YENİ KM ALGORİTMASI ---
        # Kural: Her 10.000 KM'de %5 değer kaybı.
        # Bu, her 10.000 km'de elimizde paranın %95'inin kalması demektir (0.95 ile çarpım).
        
        kac_tane_10bin = km / 10000
        
        # Matematiksel Formül: Fiyat * (0.95 üssü 10binlik_adet)
        km_carpan = 0.98 ** kac_tane_10bin
        
        fiyat_km_sonrasi = sifir_fiyat * km_carpan
        km_kayip_miktar = sifir_fiyat - fiyat_km_sonrasi

        # --- ADIM 3: YAŞ ALGORİTMASI (%3 YILLIK) ---
        # Formül: Fiyat * (0.97 üssü Yaş)
        ...
        messagebox.showinfo("Hesaplama Detayı", 
                            ...
                            f"-----------------------------------\n"
                            f"KM: {km:,} ({kac_tane_10bin:.1f} adet 10.000)\n"
                            f"KM Değer Kaybı (%5/10k): -{km_kayip_miktar:,.0f} TL\n"
                            f"Ara Toplam: {fiyat_km_sonrasi:,.0f} TL\n"
                            f"-----------------------------------\n"
                            f"Yaş: {yas}\n"
