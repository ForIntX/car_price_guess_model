...
if not df.empty:
    ...
    # --- GUI ---
    root = tk.Tk()
    root.title("Akıllı Araba Fiyatlandırma Sistemi (Extreme Mileage)")
    ...
    tk.Button(root, text="Fiyat Hesapla", command=lambda: tahmin_yap(), bg="#8e44ad", fg="white", font=("Arial", 12, "bold")).grid(row=5, column=0, columnspan=2, pady=10, sticky="ew")
    ...
    def tahmin_yap():
        ...
        # --- YENİ KM ALGORİTMASI (4 AŞAMALI) ---
        
        # 1. Aşama: 0 - 100k KM (Her 10k'da %1.5 kayıp)
        if km <= 100000:
            units = km / 10000
            km_carpan = 0.985 ** units
            
        # 2. Aşama: 100k - 1M KM (Her 10k'da %0.8 kayıp)
        elif km <= 1000000:
            ilk_100k_carpan = 0.985 ** 10 # İlk 100k tamamlandı
            kalan_units = (km - 100000) / 10000
            km_carpan = ilk_100k_carpan * (0.992 ** kalan_units)
            
        # 3. Aşama: 1M - 2M KM (ÖZEL KURAL: Her 1M'da %0.05 kayıp)
        elif km <= 2000000:
            ilk_100k_carpan = 0.985 ** 10
            ikinci_900k_carpan = 0.992 ** 90 # (900.000 / 10.000 = 90 birim)
            
            # 1M üzerindeki kısım
            fazla_km = km - 1000000
            # Milyonluk dilim hesabı
            milyon_unit = fazla_km / 1000000
            # %0.05 demek 0.0005 azalma demek -> çarpan 0.9995
            ucuncu_carpan = 0.9995 ** milyon_unit
            
            km_carpan = ilk_100k_carpan * ikinci_900k_carpan * ucuncu_carpan
            
        # 4. Aşama: 2M KM Sonrası (Fiyat Sabitlenir)
        else:
            # Tam 2.000.000 KM'deki çarpanı hesapla ve sabitle
            ilk_100k_carpan = 0.985 ** 10
            ikinci_900k_carpan = 0.992 ** 90
            ucuncu_carpan = 0.9995 ** 1 # Tam 1 milyonluk dilim (1M'den 2M'ye)
            
            km_carpan = ilk_100k_carpan * ikinci_900k_carpan * ucuncu_carpan
            # 2M'den sonraki km'ler hesaba katılmaz, çarpan değişmez.

        son_fiyat = fiyat_yas_sonrasi * km_carpan
        km_kaybi = fiyat_yas_sonrasi - son_fiyat
        
        # Eğer km > 2M ise bilgilendirme mesajı ekle
        ekstra_not = ""
        if km > 2000000:
            ekstra_not = "\n(NOT: 2 Milyon KM sınırı aşıldığı için fiyat sabitlenmiştir.)"
