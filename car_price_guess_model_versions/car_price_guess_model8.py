...
if not df.empty:
    ...
    try:
        ...
        mevcut_markalar = df['marka'].unique()
        mevcut_modeller = df['model'].unique()

        if marka_input not in mevcut_markalar:
            raise ValueError(f"'{marka_input}' markası veri setinde yok. Mevcut markalar: {', '.join(mevcut_markalar[:10])}...")
