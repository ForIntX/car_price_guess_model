...
if not df.empty:
    ...
    # Grafiği ayrı bir thread’de başlat
    grafik_thread = threading.Thread(target=plot_graph)
    grafik_thread.start()
    ...
    try:
        ...
        if marka_input not in mevcut_markalar:
            raise ValueError(f"'{marka_input}' markası veri setinde yok.")
        if model_input not in mevcut_modeller:
            raise ValueError(f"'{model_input}' modeli veri setinde yok.")
