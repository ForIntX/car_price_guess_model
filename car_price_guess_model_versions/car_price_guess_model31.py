...
if not df.empty:
    ...
    # --- GUI ---
    root = tk.Tk()
    root.title("Akıllı Araba Fiyatlandırma Sistemi (Final Versiyon)")
    ...
    tk.Button(root, text="Fiyat Hesapla", command=lambda: tahmin_yap(), bg="#2980b9", fg="white", font=("Arial", 12, "bold")).grid(row=5, column=0, columnspan=2, pady=10, sticky="ew")
    ...
    def tahmin_yap():
        ...
        # --- YENİ YAŞ ALGORİTMASI (Daha Yumuşak) ---
        mevcut_fiyat = sifir_fiyat
        for yl in range(1, yas + 1):
            if yl == 1: 
                oran = 0.90 # İlk yıl sadece %10 kayıp
            elif 2 <= yl <= 5: 
                oran = 0.94 # 2-5 yaş arası %6 kayıp
            elif 6 <= yl <= 10:
                oran = 0.96 # 6-10 yaş arası %4 kayıp
            else: 
                oran = 0.98 # 11 yaş ve üstü sadece %2 kayıp (Araba değerini korur)
