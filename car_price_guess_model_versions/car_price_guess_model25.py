...
if not df.empty:
    df = df.dropna()
    # Verileri temizleme ve tip dönüşümleri
    ...
    # Global değerler (Veri yoksa kullanılacak varsayılanlar)
    ...
    # --- 5. MOTOR GÜCÜ (DROPDOWN - YENİ) ---
    # Artık elle girilmeyecek, model seçince listeden gelecek
    ...
    def marka_secildi(event):
        ...
        model_combo.set('') 
        yas_scale.set(0)
        guc_combo.set('') # Marka değişince güç kutusunu da temizle

    def model_secildi(event):
        """
        Model seçilince:
        1. O modele ait Maksimum Yaşı bulur -> Slider'ı günceller.
        2. O modele ait Motor Güçlerini bulur -> Combobox'ı doldurur.
        """
        secilen_marka = marka_combo.get()
    ...
    def tahmin_yap():
        ...
        guc_str = guc_combo.get() # Combobox'tan veriyi al
        ...
        if not guc_str:
            messagebox.showwarning("Eksik Bilgi", "Lütfen bir Motor Gücü seçiniz.")
            return

        try:
            yas = yas_scale.get() 
            km = int(km_entry.get())
            guc = int(guc_str) # Combobox'tan gelen stringi integer'a çevir
        except ValueError:
            messagebox.showerror("HATA", "Lütfen KM alanına geçerli sayı girin!")
            return
        ...
        messagebox.showinfo("Tahmin Sonucu", 
                            ...
                            f"Model Belirsizliği: {belirsizlik_std:,.0f} TL\n"
                            f"Aralık: {alt_limit:,.0f} TL - {ust_limit:,.0f} TL\n"
                            f"\n-----------------------------------\n"
